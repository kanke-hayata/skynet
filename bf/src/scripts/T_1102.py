...
# -----------bot description-----------
#機械学習利用T_1000シリーズの4作目 一時間足ver.
#こいつからはTarget_C/Cに正しく変更されている
#単純にデータを増やしまくると予測性能下がるっぽいのでコンパクトなデータサイズで学習したものを利用
#時刻データを特徴量として利用
# -------------------------------------

if __name__ == '__main__':
    bot = Bot(balance = 0.01,name='T_1102')

    rospy.init_node('bot_T_1102',anonymous=False)
    ...
    xgbr = pickle.load(open("/root/model/T_1102/xgbr.pickle","rb"))
    ...
    while True:
        if 1 < time.time() % 3600 and time.time() % 3600 < 11:
            ...
            if type(state) == dict and 'state' in state.keys() and state['state'] == 'RUNNING' and state['health'] in ['NORMAL','BUSY','VERY BUSY']:
                ...
                latest = df[-1:][['MACDS/O','UP','FirstUP','C/O','C-1/C','H/O','L/O','Hour']]
                ...
                print(df[-1:]['timestamp'].values[0],df[-1:]['Open'].values[0],df[-1:]['Close'].values[0],df[-1:]['Hour'].values[0])
                ohlc = utils.get_ohlc(periods=3600,datasize=100)
                print(ohlc[-1:]['timestamp'].values[0],ohlc[-1:]['Open'].values[0],ohlc[-1:]['Close'].values[0],df[-1:]['timestamp'].values[0]//3600%24)
                time.sleep(2)
                bot.info()
